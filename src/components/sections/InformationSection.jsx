    ...
    <section style={{ background:T.teal, padding:"88px 0", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ maxWidth:800, margin:"0 auto", padding:"0 24px", textAlign:"center", position:"relative", zIndex:1 }}>
        ...
        <div style={{ minHeight:160 }}>
          <p style={{ fontFamily:"Fraunces, serif", fontSize:"clamp(1.15rem,2.5vw,1.5rem)", fontStyle:"italic",
        ...
        <div style={{ display:"flex", justifyContent:"center", gap:8, marginTop:32 }}>
          {TESTIMONIALS.map((_,i) => (
            <button key={i} onClick={() => setCur(i)}
              style={{ width: i===cur ? 28 : 8, height:8, borderRadius:50,
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))", gap:0, position:"relative" }}>
          {steps.map((s, i) => (
            <div key={s.n} style={{ textAlign:"center", padding:"32px 20px", position:"relative" }}>
              {i < steps.length-1 && (
                <div style={{ position:"absolute", top:48, right:0, width:"50%", height:2,
              ...
              <div className="serif" style={{ fontSize:".75rem", fontWeight:700, color:T.orange,
                letterSpacing:".1em", textTransform:"uppercase", marginBottom:6 }}>Step {s.n}</div>
          ...
          {FAQS.map((f, i) => (
            <div key={i} style={{ background:"#fff", borderRadius:14, overflow:"hidden",
              ...
              <button onClick={() => setOpen(open===i ? null : i)}
                ...
                {f.q}
                <span style={{ fontSize:20, color:T.orange, transition:"transform .3s",
                  transform: open===i ? "rotate(45deg)" : "rotate(0)", flexShrink:0 }}>+</span>
              </button>
              <div className="accordion-body" style={{ maxHeight: open===i ? 200 : 0,
                opacity: open===i ? 1 : 0 }}>
